...
bool I2C_WriteOneBit(uint8_t DevAddr, uint8_t RegAddr, uint8_t BitNum,
                     uint8_t Data) {
  uint8_t Dat;
...
bool I2C_WriteBits(uint8_t DevAddr, uint8_t RegAddr, uint8_t BitStart,
...
unsigned char I2C_WriteOneByte(uint8_t DevAddr, uint8_t RegAddr, uint8_t Data) {
  I2C_Start();
...
bool I2C_WriteBuff(uint8_t DevAddr, uint8_t RegAddr, uint8_t Num,
                   uint8_t *pBuff) {
  uint8_t i;
...
bool I2C_ReadBuff(uint8_t DevAddr, uint8_t RegAddr, uint8_t Num,
                  uint8_t *pBuff) {
  uint8_t i;
...
//					length：Number of bytes；*data:The
// pointer where the read data will be stored Output  : count：Number of bytes
// read out-1
// 函数功能：IIC连续读数据
...
//**************************************************************************/
// unsigned char IICreadBytes(unsigned char dev, unsigned char reg, unsigned
...
////带ACK的读数据 		 else data[count]=IIC_Read_Byte(0);
////最后一个字节NACK
//	}
//     IIC_Stop();//产生一个停止条件
//     return count;
