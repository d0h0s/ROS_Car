...
void Robot_Select(void) {
  // The ADC value is variable in segments, depending on the number of car
  // models. Currently there are 6 car models, CAR_NUMBER=6
  // ADC值分段变量，取决于小车型号数量，目前有6种小车型号，CAR_NUMBER=6
  Divisor_Mode = 2048 / CAR_NUMBER + 5;
  Car_Mode = (int)((get_potentiometer_ADC_voltage()) /
                   Divisor_Mode); // Collect the pin information of
                                  // potentiometer //采集电位器引脚信息
  if (Car_Mode > 5)
    Car_Mode = 5;
  ...
  switch (Car_Mode) {
  case Mec_Car:
    Robot_Init(MEC_wheelspacing, MEC_axlespacing, 0, HALL_30F,
               Photoelectric_500, Mecanum_75);
    break; // 麦克纳姆轮小车
  case Omni_Car:
    Robot_Init(0, 0, Omni_Turn_Radiaus_109, HALL_30F, Photoelectric_500,
               FullDirecion_60);
    break; // 全向轮小车
  case Akm_Car:
    Robot_Init(Akm_wheelspacing, Akm_axlespacing, 0, HALL_30F,
               Photoelectric_500, Black_WheelDiameter);
    break; // 阿克曼小车
  case Diff_Car:
    Robot_Init(Diff_wheelSpacing, 0, 0, HALL_30F, Photoelectric_500,
               Black_WheelDiameter);
    break; // 两轮差速小车
  case FourWheel_Car:
    Robot_Init(Four_Mortor_wheelSpacing, Four_Mortor__axlespacing, 0, HALL_30F,
               Photoelectric_500, Black_WheelDiameter);
    break; // 四驱车
  case Tank_Car:
    Robot_Init(Tank_wheelSpacing, 0, 0, HALL_30F, Photoelectric_500,
               Tank_WheelDiameter);
    break; // 履带车
  }
...
Input   : wheelspacing, axlespacing, omni_rotation_radiaus, motor_gear_ratio,
Number_of_encoder_lines, tyre_diameter Output  : none 函数功能：初始化小车参数
                ...
                float gearratio, float Accuracy, float tyre_diameter) //
{
  ...
  // 电机(车轮)转1圈对应的编码器数值
  Encoder_precision = EncoderMultiples * Robot_Parament.EncoderAccuracy *
